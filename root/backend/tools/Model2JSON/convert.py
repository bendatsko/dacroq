...
def create_benchmark_json(
    ...
    solved_problems = sum(1 for p in problems if not p.get("is_unsolved", False))
    avg_cycles = np.mean([p["tts_sim_cycles"] for p in problems if p["tts_sim_cycles"] > 0]) if problems else 0
    avg_power = np.mean([p["power_mw"] for p in problems if p["power_mw"] > 0]) if problems else 0
    ...
    overview = {
        ...
        "success_rate": f"{(solved_problems / total_problems * 100):.1f}%" if total_problems > 0 else "0%",
        "avg_cycles": int(avg_cycles),
        "avg_power_mw": f"{avg_power:.2f}",
    ...
    # Return both benchmarks and overview
    return {
        "overview": overview,
...
def process_batch(
    ...
    # Sort benchmarks by instance index
    result["benchmarks"].sort(key=lambda x: x["instance_idx"])
    ...
    print(f"Successfully created benchmark data for {len(result['benchmarks'])} problems.")
