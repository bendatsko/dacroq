...
const EnhancedTestTable: React.FC<EnhancedTestTableProps> = ({
  ...
  // If the value is a Firebase serverTimestamp placeholder, display "Pending..."
  const formatDate = (dateValue: any) => {
    if (!dateValue) return "Pending...";
    let date: Date;

    // If the object has a toDate method, use it (Firestore Timestamp)
    if (typeof dateValue === "object" && typeof dateValue.toDate === "function") {
      date = dateValue.toDate();
    } else if (typeof dateValue === "object" && "seconds" in dateValue) {
      // If it has a seconds property, use that
  ...
  }, [columns, selectedTests, currentUser, adminMode, handleViewResults, filteredTests]);
