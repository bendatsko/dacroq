...
Pure hardware interface for the Dacroq platform running on the lab server.
Handles all physical hardware operations including:
- Teensy 4.1 LDPC decoder communication  
...
- GPIO-based hardware reset control
...
def stop_daemon():
    ...
    if not running_pid:
        print("❌ No running Dacroq Hardware API daemon found")
        return False
    
    try:
        print(f"🛑 Stopping Dacroq Hardware API daemon (PID: {running_pid})...")
...
def start_daemon():
    ...
    running_pid = get_running_pid()
    if running_pid:
        print(f"⚠️ Dacroq Hardware API daemon already running (PID: {running_pid})")
        return False
    
    print("🚀 Starting Dacroq Hardware API daemon...")
    ...
    logger.info("=" * 60)
    logger.info("🔧 Dacroq Hardware API Daemon Starting")
